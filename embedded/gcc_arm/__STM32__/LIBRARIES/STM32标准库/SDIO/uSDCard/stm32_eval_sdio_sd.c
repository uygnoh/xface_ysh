...
SD_Error SD_Init(void)
{
        ...
        // 上电识别， 卡初始化完成后， 进入数据传输模式，提高读写速度
        // SDIO_CK = 72MHz / (2 + SDIO_TRANSFER_CLK_DIV) = 24MHz
...
SD_Error SD_PowerON(void)
{
        ...
        // CMD0: 复位所有SD_CARD， 进入空闲状态（没有响应）
        SDIO_CmdInitStructure.SDIO_Argument = 0x0;                  // 参数
        SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE; // 命令号码
        SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;     // 表示无响应
        SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;             // 不等待
        SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;         // 开启命令状态机
        ...
        if (errorstatus == SD_OK) {
                ...
                while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL)) {
                        ...
                        // 若卡需求电压在SDIO的供电电压范围， 会自动上电并标志(pwr_up)位
                        // 读取卡寄存器，卡状态
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
        ...
        // 加入下面这一段代码， 没有这一段容易卡在DMA检测中
        // CMD16: 设置块长度
        // ________________________________________________________
        SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)BlockSize;     // 块大小
        SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;      // 命令
        SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
        SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
        SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
        SDIO_SendCommand(&SDIO_CmdInitStructure);
        ...
        SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToSDIO; // 从卡到SDIO
...
SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
        ...
        // 在SDSC卡， 地址以字节为单位 ！！！
        // 在SDHC卡， 地址单位为块地址， 每块（512字节），所以要 / 512
        if (CardType == SDIO_HIGH_CAPACITY_SD_CARD) {
                startaddr /= 512;
                endaddr   /= 512;
        }
