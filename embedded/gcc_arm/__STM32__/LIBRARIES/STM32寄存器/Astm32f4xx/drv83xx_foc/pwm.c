// PWM频率设计参考依据：
// http://www.emcu.it/STM32/STM32-MotorControl/STM32-MotorControl.html#Presentation_of_the_FOC_controller
//
//      MF = mechanical frequency
//      MF = rpm/60（每秒转速）
//      MF = 20000/60 = 333.33 per second
//
//      Hz = MF x Number_of_Pole（极对数）
//      Hz = 333.33 x 4 = 1333.33Hz
//
//      PWM Frequency:
//      Fpwm >= 1333.33 *12 = 16KHz
//
//
//
//
...
void pwm_setup(void)
{
        ...
        ////////////////////////////////////////////////////////////////////////
        // TIM1_CH4 调试使用，正常使用的时候置（0）
        // 测试TIM1_CH4通道上的脉冲宽度
        // 把（PA11）的引脚引出来，测试脉冲宽度
        #if 1   
        RCC->AHB1ENR    |=  (0x01);
        GPIOA->MODER    &= ~(0x03 << 22);
        GPIOA->MODER    |=  (0x02 << 22);
        ...
        TIM1_>CR1       |=  (0x01 << 5);        // 采用中心对齐模式1，其实采用1，2，3模式都可以
                                                // 代表向上或向下计数时候CNT与CCR寄存器产生中断的时机
        TIM1->CR1       &= ~(0x01 << 3);        // 不采用单脉冲模式
        TIM1->CR1       &= ~(0x03 << 1);        // 更新事件请求源选择（bit2=0, bit1=0）
        ...
        //
        //      PWM = 20Khz     TIM1 = 40Khz
        //      电流采样，以据电路图，必须在（下管导通区间采集）
        //
        //
        TIM1->CCR4       =  (50);             
        ...
        // 死区时间设置为
        // （DTG[7:5]= 0xx => DT=DTG[7:0]x t_dtg ,其中 t_dtg =t_DTS）
