...
void timer3_hall_setup(void)
{
        ...
        // PC7  TIM3-CH1        // 复用功能寄存器配置为（AF2）
        // PC6  TIM3-CH2        // 复用功能寄存器配置为（AF2）
        GPIOC->AFR[0]   &=  (0x00FFFFFF);
        GPIOC->AFR[0]   |=  (0x22000000);
        // PC8  TIM3-CH3        // 复用功能寄存器配置为（AF2）
        GPIOC->AFR[1]   &=  (0xFFFFFFF0);
        GPIOC->AFR[1]   |=  (0xFFFFFFF2);
        ...
        RCC->APB1RSTR   |=  (0x01 << 1);        // 复位TIM3全部寄存器
        RCC->APB1RSTR   &= ~(0x01 << 1);        // 结束复位
        
        
        // TIM3 输入滤波器配置（input filter & edge detector）
        ...
        TIM3->CR1       &= ~(0x03 << 5);        // 递增递减模式
        TIM3->CR1       &= ~(0x01 << 4);        // 计数器递增计数
        TIM3->CR1       &= ~(0x01 << 3);        // 连续计数模式
        //  bit2 = 0    URS:  Update request source
        //  bit1 = 0    UDIS: Update disable
