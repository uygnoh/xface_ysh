...
void i2c_is_busy(void)
{
        ...
        while ((I2C2->SR2 & (1 << 1)) != 0) {           // 总线忙BUSY !=0 就在这里循环等待
                RCC->APB2ENR    |= (0x00000008);        // [3]  GPIOB时钟使能
                GPIOB->CRH      &= (0xFFFF00FF);        // ^0   PB10 PB11
                GPIOB->CRH      |= (0x00007700);        // PB10-SCK : 开漏输出
                                                        // PB11-SDA : 开漏输出
...
void i2c_setup(void)
{
        ...
        GPIOB->CRH      |= (0X0000FF00);        // PB10-SCK : 开漏复用输出
                                                // PB11-SDA : 开漏复用输出
