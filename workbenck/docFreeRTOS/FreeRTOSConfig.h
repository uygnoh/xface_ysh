...
//系统可管理的最高中断优先级
//表示用户可以在抢占式优先级为5到15的中断里面调用FreeRTOS的API函数
#define configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY    (5)

//设置内核中断优先级
//即SysTick和PendSV的中断优先级就是240
#define configKERNEL_INTERRUPT_PRIORITY \
( configLIBRARY_LOWEST_INTERRUPT_PRIORITY << (8 - configPRIO_BITS) )

//即宏定义configMAX_SYSCALL_INTERRUPT_PRIORITY的数值。 这个数值是赋值给寄存器basepri使用的
//configMAX_SYSCALL_INTERRUPT_PRIORITY 是宏configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY左移４位而来的，
//此宏设置好之后，低于此优先级的中断可以安全的调用FreeRTOS的API函数，高于此优先级的中断FreeRTOS是不能禁止的，中断服
