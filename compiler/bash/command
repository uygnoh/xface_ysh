...
### md5su 校验文件完整性
```note
md5sum file_name    # 文件校验
cat /etc/shells     # 查看系统内置shell
uname -a            # 查看系统版本
...
CTRL+C              # 结束当前命令
CTRL+L              # 清屏并重新显示
CTRL+Z              # 暂停前台进程返回 bash，需要时可用 fg 将其切换回前台bg
```
...
num=$((1 + (2 + 3) * 2))  # 复杂计算lare -F                
declar                    # 查看所有函数，仅显示函数名
```
